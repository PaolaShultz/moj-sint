...
inline int exponentOfFloat(float value) {
  std::uint32_t bits;
  std::memcpy(&bits, &value, sizeof(bits));
  return static_cast<int>((bits >> 23) & 0xff) - 127;
}
#define EXPOFFLT(value) exponentOfFloat(value)
inline int exponentOfDouble(double value) {
  std::uint64_t bits;
  std::memcpy(&bits, &value, sizeof(bits));
  return static_cast<int>((bits >> 52) & 0x7ff) - 1023;
}
#define EXPOFDBL(value) exponentOfDouble(value)
