...
{
  // Local repair: these are actual double arrays, not Complex objects.
  setDirection(FORWARD);
  for(int n=0; n<N; ++n)
    reAndIm[n] = normalizationFactor != 1.0 ? signal[n] * normalizationFactor : signal[n];
  rdft(N, 1, reAndIm, ip, w);
  for(int n=3; n<N; n+=2)
    reAndIm[n] = -reAndIm[n];
}
...
{
  // Local repair: preserve the original arithmetic without a type-punned view.
  setDirection(INVERSE);
  for(int n=0; n<N; ++n)
    signal[n] = normalizationFactor != 1.0 ? 2.0 * reAndIm[n] * normalizationFactor : 2.0 * reAndIm[n];
  for(int n=3; n<N; n+=2)
    signal[n] = -signal[n];
  rdft(N, -1, signal, ip, w);
}
