...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    /** Destructor. */
    ~Open303();
    Open303(const Open303&) = delete;
    Open303& operator=(const Open303&) = delete;

    // Clears voice state without reallocating or rebuilding tables.
    void reset();
    ...
    void setTuning(double newTuning);
    ...
    void setAccentDecay(double newAccentDecay) { accentDecay = newAccentDecay; if(currentNote >= 0 && currentVel >= 100) setMainEnvDecay(accentDecay); }
  ...
  protected:
    ...
    /** Triggers a detached note. */
    void triggerNote(int noteNumber, bool hasAccent);

    /** Changes a held-note pitch without retriggering envelopes. */
    ...
    struct HeldNote { int key; int velocity; };
    HeldNote heldNotes[128]{};
    int numHeldNotes = 0;
    unsigned quietSamples = 0;
  ...
  INLINE double Open303::getSample()
  {
    ...
    // Require a sustained quiet release before clearing every state.
    if (numHeldNotes == 0 && !ampEnv.isNoteOn() &&
        fabs(ampEnvOut) < 1.0e-9 && fabs(tmp) < 1.0e-9)
      ++quietSamples;
    else
      quietSamples = 0;
    if (quietSamples >= 64) { reset(); return 0.0; }
