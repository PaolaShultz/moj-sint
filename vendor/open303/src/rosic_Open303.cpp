...
{
  if (!std::isfinite(newSampleRate) || newSampleRate < 40000.0) return;
...
void Open303::setAccent(double newAccent)
{
  ...
  if (currentNote >= 0) accentGain = currentVel >= 100 ? accent : 0.0;
}

void Open303::setTuning(double newTuning)
{
  tuning = newTuning;
  if (currentNote >= 0) oscFreq = pitchToFreq(currentNote, tuning);
}

void Open303::setDecay(double newDecay)
{
  normalDecay = newDecay;
  if (currentNote >= 0 && currentVel < 100) setMainEnvDecay(normalDecay);
}
...
void Open303::noteOn(int noteNumber, int velocity, double /*detune*/)
{
  ...
  for (int i = 0; i < numHeldNotes; ++i)
    if (heldNotes[i].key == noteNumber) { found = i; break; }
  if (velocity == 0 && found < 0) return; // stale release, including before first note
  const bool hadNotes = numHeldNotes != 0;
  const bool releasedLatest = found == numHeldNotes - 1;
  if (found >= 0) {
    for (int i = found; i + 1 < numHeldNotes; ++i) heldNotes[i] = heldNotes[i+1];
    --numHeldNotes;
  }
  if (velocity > 0) {
    // With unique MIDI keys, removing a duplicate leaves room even at capacity.
    ...
    currentVel = 0;
    ampEnv.noteOff();
  } else if (releasedLatest) {
    const HeldNote note = heldNotes[numHeldNotes-1];
    currentNote = note.key;
    currentVel = note.velocity;
    slideToNote(note.key, note.velocity >= 100);
  }
  quietSamples = 0;
}
...
void Open303::reset()
{
  ...
  filter.reset();
  highpass1.reset(); highpass2.reset(); allpass.reset(); notch.reset();
  antiAliasFilter.reset(); ampDeClicker.reset();
  rc1.reset(); rc2.reset(); pitchSlewLimiter.reset();
  mainEnv.reset(); ampEnv.reset();
}
...
void Open303::updateNormalizer1() { n1 = 1.0; }
void Open303::updateNormalizer2() { n2 = 1.0; }
